...
public class Bugpath {
    ...
    boolean bugPath(){
        ...
        if (uc.getType() != UnitType.ANT && myLoc.distanceSquared(target) <= uc.getType().attackRangeSquared && !uc.isObstructed(myLoc, target)){
        ...
        if (uc.getType() == UnitType.ANT && uc.getRound() < 100) uc.println("Moving2!");
    ...
    int getAttackRange(UnitType type){
        ...
        //if (this.type != UnitType.SPIDER) return type.getAttackRangeSquared();
